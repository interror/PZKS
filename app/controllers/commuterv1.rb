class Commute
	...
	def initialize(string, options2)
		@string = string
		@res_shuffle = []
		options1 = "-b"
		run_program(options1, options2)
	end

	def run_program(options1, options2)
		@hash_of_brackets = {}
		@counter_breckets = 1
		...
		shuffle_tokens(super_array, options2)

	end

	def shuffle_tokens(super_array, options2)
	shuffle_hash = {}
	...
	buf_array = []
		...
		find_max_num_of_shuffle = 0
		shuffle_hash[start] = buf_array
		options2.to_i.times.each do |t|
			output_string = []
			shuffle_hash.each do |key,val|
				if key > 0
					val.shuffle!
					if val.length >= find_max_num_of_shuffle
						find_max_num_of_shuffle = val.length
					end
				end
			end
			p find_max_num_of_shuffle
